...
def is_latest():
    ...
    lastest_version = urllib.urlopen('http://www.easeserver.org/lastest_version.html?jsn=%s' % jsn).read().strip()
...
def install(request):
    ...
    if User.objects.filter(id=5000):
        return http_error(request, 'easeserver已初始化，不能重复安装！')
    ...
    User(id=5000, username="admin", password=PyCrypt.md5_crypt('admin'),
         name='admin', email='admin@easeserver.org', role='SU', is_active=True, dept=dept).save()
    return http_success(request, u'easeserver初始化成功')
